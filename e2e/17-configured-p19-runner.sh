#!/usr/bin/env bash
# 17-configured-p19-runner.sh — unmanaged protocol-19 harness contract.
...
out="$(dirname "$BOARD_SOCKET")/p19-runner-$BOARD_RUN_ID.json"
...
PY
"$BOARD_BIN" comment "p19 runner: exact startup/env evidence recorded"
...
sleep "${P19_RUNNER_HOLD:-60}"
RUNNER
chmod 700 "$RUNNER"
e2e_script_resource_register configured-runner p19-configured-runner "$RUNNER" \
  || fail "cannot record configured runner digest"
e2e_defer "e2e_script_remove_owned p19-configured-runner '$RUNNER'"
...
PY
e2e_daemon_start
e2e_ws_create p19-runner; WS_ID="$E2E_WS"
...
EXEC_ID="$(col_create '{"name":"P19 Runner","trigger":"auto"}')"

step "Dispatch configured harness through the protocol-19 runner bridge"
card_json="$($BOARD_BIN card new --title 'P19 configured' \
  --description $'configured prompt with spaces\nand a newline' --harness p19-runner --effort low \
  --space-kind workspace --space-ref "$WS_ID" --json)"
CARD_ID="$(printf '%s' "$card_json" | jget id)"
mut "board move $CARD_ID 'P19 Runner' -> unmanaged pane run bridge"
e2e_board_herdr_mutate -- move "$CARD_ID" "P19 Runner" --json >/dev/null
outcome="$(wait_ok "$CARD_ID" 100)" || {
  printf '%s\n' '--- configured runner failure diagnostics (disposable session only) ---' >&2
  e2e_card_failure_diag "$CARD_ID"
  printf 'runner_records=%s\n' "$(find "$E2E_TMP" -maxdepth 1 -type f -name 'p19-runner-*.json' -printf . 2>/dev/null | wc -c)" >&2
...
RECORD="$E2E_TMP/p19-runner-$RUN_ID.json"
SHOW="$E2E_TMP/p19-runner-show.json"
...
assert any(re.search(rf"card-{card}-p19-runner(?:-r\d+)?$", p.get("label") or "") for p in kp)
...
panes=json.load(sys.stdin).get("panes", [])
card=sys.argv[1]
matched=[p for p in panes if re.search(rf"card-{re.escape(card)}-p19-runner(?:-r\d+)?$", p.get("label") or "")]
...
echo "17-configured-p19-runner: argv/env bridge, bounded-held layout, and explicit done contract passed"
