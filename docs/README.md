...
|---|---|---|
| [install.md](install.md) | Installation details beyond the README's one-liner: a custom CLI install directory and its managed-checksum marker, adding a Herdr keybinding, installing harness integrations (Pi/Claude/Codex) and the optional agent skill, and how named Herdr sessions register the plugin. | are installing or reinstalling herdr-board, or want the optional integration/keybinding/skill setup. |
| [configuration.md](configuration.md) | Platform config path: top-level and typed `[daemon]` settings, config-defined `[harness.NAME]` adapters (argv placeholders, capability catalogs, `resume`), parse/override precedence, and every environment variable. | are tuning the daemon, adding a config-defined harness, or need what an environment variable does. |
...
| [herdr.md](herdr.md) | How to learn and verify **Herdr** facts (there is no man page): the live sources of truth (`herdr api schema --json`, `herdr <cmd> --help`, `herdr api snapshot`), per-harness agent integrations, and the exact compatibility gate (Herdr 0.8.0 / protocol 19 only; no protocol-17 path). | hit a Herdr command/shape that misbehaves, or need to confirm what the installed Herdr actually does. |
| [testing.md](testing.md) | The testing pyramid in this repo (unit/pure → daemon+CLI integration → TUI snapshots → live E2E), how the provider-free fake Pi/Claude/Codex suite works (including protocol-19 scenarios 16/17), and how to write a scenario. The use case ↔ scenario catalog lives in [`../e2e/README.md`](../e2e/README.md). | are adding a feature and need to test it, or are writing/running the live E2E suite. |
...
files, that the version matrix above says schema v13 / protocol v1 / Herdr 0.8.0-protocol 19, and that
...
succeed, installs SHA-verified Herdr 0.8.0, and runs the wrapper above. This keeps cheap failures
...
0.8.0 Linux x86_64 binary, verifies protocol 19, runs `e2e/run-all.sh --require-all`, and exports
