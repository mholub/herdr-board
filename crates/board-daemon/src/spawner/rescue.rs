...
///   pinned protocol-19 schema gives `AgentInfo` **both** an `agent` and a
///   separate `name` field, and `e2e/16-managed-p19.sh` matches `pane.agent`
...
/// protocol-19 schema. The `agent.start` name is deliberately *not* matched
...
pub(crate) fn rescue_run_pane(plan: &RescuePlan<'_>) -> anyhow::Result<RescueOutcome> {
    ...
    // Protocol 19 never inherits a workspace cwd, so the rescue pane's cwd is
