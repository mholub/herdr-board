...
#[test]
fn protocol19_idle_after_done_does_not_rearm_an_awaiting_run() {
    let (d, run_id, card_id, _events) = active_daemon();

    // Protocol 19 may emit the terminal turn's `done` followed by `idle`.
    ...
    assert!(
        d.sched
            ...
            .idle_since
            .is_none(),
        "a trailing protocol-19 idle event must not rearm idle expiry while awaiting",
    );
}
